...
public:
    ...
    void init()
    {
        ...
        // Add this to the end of your init() function, after GPIO setup:
        std::cout << "Enabling keypad interrupts..." << std::endl;
        // write_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_INTENSET, { 0xFF, 0xFF });
        write_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_INTENSET, { 0x01 });

        // Instead, add this to disable interrupts:
        // std::cout << "Disabling keypad interrupts (using polling mode)..." << std::endl;
        // write_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_INTENSET, { 0x00, 0x00 });
    }
    ...
    // Replace your activateKey function with this:
    void activateKey(uint8_t key, uint8_t edge, bool enable = true)
    {
        // Convert trellis key to seesaw key numbering
        uint8_t seesaw_key = NEO_TRELLIS_KEY(key);

        keyState ks;
        ks.reg = 0; // Clear all bits first
        ks.bit.STATE = enable ? 1 : 0;
        ks.bit.ACTIVE = enable ? (1 << edge) : 0;

        std::cout << "Activating key " << (int)key << " (seesaw " << (int)seesaw_key
                  << ") edge " << (int)edge << " state " << (int)ks.reg << std::endl;

        write_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_EVENT, { seesaw_key, ks.reg });
    }
    ...
    //     if (count > 0 && count < 255) {
    //         std::cout << "Keys count: " << (int)count << std::endl;
    //         // Read the events from FIFO - each event is 1 byte
    //         std::vector<uint8_t> data = read_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_FIFO, count);

    //         for (uint8_t i = 0; i < count; i++) {
    //             keyEventRaw event;
    //             event.reg = data[i];

    //             // Convert from seesaw key numbering to trellis key numbering
    //             uint8_t key_num = NEO_TRELLIS_SEESAW_KEY(event.bit.NUM);

    //             if (key_num < NEO_TRELLIS_NUM_KEYS) {
    //                 std::cout << "Key " << (int)key_num << " edge " << (int)event.bit.EDGE << std::endl;

    //                 // Call callback if registered
    //                 if (callbacks[key_num]) {
    //                     KeyEvent evt;
    //                     evt.number = key_num;
    //                     evt.edge = event.bit.EDGE;
    //                     callbacks[key_num](evt);
    //                 }
    //             } else {
    //                 // std::cout << "++++ Key " << (int)key_num << " edge " << (int)event.bit.EDGE << std::endl;
    //             }
    ...
    // Replace your keyEventRaw union with this:
    union keyEventRaw {
        struct {
            uint8_t NUM : 6; ///< the key number (bits 0-5)
            uint8_t EDGE : 2; ///< the edge type (bits 6-7)
        } bit;
        uint8_t reg;
    };

    // Replace your readKeys function with this version that filters invalid events:
    void readKeys(bool polling = true)
    {
        uint8_t count = getKeypadCount();
        ...
        if (count > 0 && count < 255) {
            ...
            // Read the events from FIFO
            std::vector<uint8_t> data = read_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_FIFO, count);

            for (uint8_t i = 0; i < count; i++) {
                uint8_t raw_byte = data[i];

                // Skip invalid 0xFF bytes
                if (raw_byte == 0xFF) {
                    // std::cout << "Skipping invalid event: 0xFF" << std::endl;
                    continue;
                }

                // Manual bit extraction
                uint8_t key_num_raw = raw_byte & 0x3F; // Lower 6 bits
                uint8_t edge_raw = (raw_byte >> 6) & 0x03; // Upper 2 bits

                std::cout << "Raw byte: 0x" << std::hex << (int)raw_byte << std::dec
                          << " -> key_raw=" << (int)key_num_raw
                          << " edge=" << (int)edge_raw << std::endl;

                // Convert from seesaw key numbering to trellis key numbering
                uint8_t key_num = NEO_TRELLIS_SEESAW_KEY(key_num_raw);

                if (key_num < NEO_TRELLIS_NUM_KEYS) {
                    std::cout << "==> Key " << (int)key_num << " edge " << (int)edge_raw;
                    if (edge_raw == EDGE_RISING)
                        std::cout << " (PRESS)";
                    else if (edge_raw == EDGE_FALLING)
                        std::cout << " (RELEASE)";
                    std::cout << std::endl;
                }
            }
    ...
    void clearInit()
    {
        // Also modify your main() to clear the FIFO first:
        // Add this after trellis creation and before activating keys:
        std::cout << "Clearing keypad FIFO..." << std::endl;
        uint8_t initial_count = getKeypadCount();
        if (initial_count > 0) {
            std::vector<uint8_t> dummy = read_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_FIFO, initial_count);
            std::cout << "Cleared " << (int)initial_count << " stale events" << std::endl;
        }
    }

    void activateKeys()
    {
        // // Replace the key activation loop with:
        // std::cout << "Activating all keys for rising and falling edges..." << std::endl;
        // for (int i = 0; i < 16; i++) {
        //     // Enable both press and release events for each key
        //     keyState ks_press, ks_release;

        //     // Press events (rising edge)
        //     ks_press.reg = 0;
        //     ks_press.bit.STATE = 1;
        //     ks_press.bit.ACTIVE = (1 << EDGE_RISING);
        //     write_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_EVENT,
        //         { (uint8_t)NEO_TRELLIS_KEY(i), ks_press.reg });

        //     // Release events (falling edge)
        //     ks_release.reg = 0;
        //     ks_release.bit.STATE = 1;
        //     ks_release.bit.ACTIVE = (1 << EDGE_FALLING);
        //     write_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_EVENT,
        //         { (uint8_t)NEO_TRELLIS_KEY(i), ks_release.reg });

        //     std::this_thread::sleep_for(std::chrono::milliseconds(10));
        // }

        // First, disable ALL key events to stop spurious triggers
        std::cout << "Disabling all key events..." << std::endl;
        for (int i = 0; i < 64; i++) { // Disable all possible keys (0-63)
            keyState ks_disable;
            ks_disable.reg = 0; // STATE=0, ACTIVE=0 disables the key
            write_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_EVENT,
                { (uint8_t)i, ks_disable.reg });
        }

        // Clear any remaining events in FIFO
        std::cout << "Clearing FIFO after disable..." << std::endl;
        uint8_t clear_count = getKeypadCount();
        if (clear_count > 0) {
            auto dummy = read_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_FIFO, clear_count);
            std::cout << "Cleared " << (int)clear_count << " events" << std::endl;
        }

        std::cout << "Enabling specific keys (0-15) for press/release..." << std::endl;
        for (int i = 0; i < 16; i++) {
            uint8_t seesaw_key = NEO_TRELLIS_KEY(i);

            // Enable press events (rising edge)
            keyState ks_press;
            ks_press.reg = 0;
            ks_press.bit.STATE = 1;
            ks_press.bit.ACTIVE = (1 << EDGE_RISING);
            write_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_EVENT,
                { seesaw_key, ks_press.reg });

            // Enable release events (falling edge)
            keyState ks_release;
            ks_release.reg = 0;
            ks_release.bit.STATE = 1;
            ks_release.bit.ACTIVE = (1 << EDGE_FALLING);
            write_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_EVENT,
                { seesaw_key, ks_release.reg });

            std::cout << "Enabled key " << i << " (seesaw key " << (int)seesaw_key << ")" << std::endl;
            std::this_thread::sleep_for(std::chrono::milliseconds(5));
        }

        // Final FIFO clear
        std::cout << "Final FIFO clear..." << std::endl;
        clear_count = getKeypadCount();
        if (clear_count > 0) {
            auto dummy = read_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_FIFO, clear_count);
            std::cout << "Cleared " << (int)clear_count << " final events" << std::endl;
        }

        std::cout << "Keypad setup complete. Press keys to test..." << std::endl;
    }
};

...
int spiWrite(char* buf, unsigned count)
{
    int err;
    ...
    err = ioctl(f_dev, SPI_IOC_MESSAGE(1), &spi);
...
int spiXfer(char* txBuf, char* rxBuf, unsigned count)
{
    int err;
    struct spi_ioc_transfer spi;
...
int spiRead(char *buf, unsigned count)
{
   int err;
   struct spi_ioc_transfer spi;
...
void sendCmd(uint8_t cmd, uint8_t* payload, uint32_t payloadSize)
{
    ...
    if (payloadSize > 0) {
        ...
        spiWrite((char*)payload, payloadSize);

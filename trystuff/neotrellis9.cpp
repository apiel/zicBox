...
public:
    ...
    std::vector<uint8_t> read_register(uint8_t regHigh, uint8_t regLow, uint8_t length)
    {
        ...
        while (pos < length) {
            ...
            if (::write(i2c_fd, prefix, sizeof(prefix)) != sizeof(prefix)) {
                std::cerr << "I2C register‑select write failed: " << strerror(errno) << " (errno " << errno << ")\n";
                throw std::runtime_error("I2C register-select write failed");
            }

            ssize_t got = ::read(i2c_fd, buf + pos, to_read);
            if (got != static_cast<ssize_t>(to_read)) {
                std::cerr << "I2C read failed: " << strerror(errno) << " (errno " << errno << ")\n";
    ...
    void setKeypadEvent(uint8_t key, uint8_t edge, bool enable = true)
    {
        ...
        this->write_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_EVENT, { key, ks.reg });
    }
    ...
    uint8_t read8(uint8_t regHigh, uint8_t regLow)
    {
        return read_register(regHigh, regLow, 1)[0];
    }
    ...
    std::vector<uint8_t> readKeypad(keyEventRaw* buf, uint8_t count)
    {
        return read_register(SEESAW_KEYPAD_BASE, SEESAW_KEYPAD_FIFO, count);
    }
    void read(bool polling = true)
    {
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                e.bit.NUM = NEO_TRELLIS_SEESAW_KEY(e.bit.NUM);
                if (e.bit.NUM < NEO_TRELLIS_NUM_KEYS) {
                    ...
                    if (e.bit.EDGE == SEESAW_KEYPAD_EDGE_RISING) {
                        ...
                        std::cout << "Pressed: " << (int)e.bit.NUM << std::endl;
                    } else if (e.bit.EDGE == SEESAW_KEYPAD_EDGE_FALLING) {
                        // or is the pad released?
                        // trellis.pixels.setPixelColor(evt.bit.NUM, 0); //off falling
                        std::cout << "Released: " << (int)e.bit.NUM << std::endl;
                    }
                }
            }
        }

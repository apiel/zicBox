...
// Internal Menu Params
Param slotParam { "Slot", .value = 1.0f, .min = 1.0f };
...
public:
    SFMLEmulator(DrawMono<W, H>& d, int pixelScale)
        : drawer(d), scale(pixelScale), window(sf::VideoMode(W * pixelScale, H * pixelScale), "Kick2 System")
    {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                ...
                if (event.key.code == sf::Keyboard::A) {
                    std::lock_guard<std::mutex> lock(engine_mutex);
                    kick2.noteOn(60, 1.0f); v_meter = 1.0f;
                }
            ...
            if (event.type == sf::Event::MouseWheelScrolled) {
                ...
                if (is_menu_open) {
                    if (col == 0) {
                        int next = menu_scroll_idx + rawDir;
                        menu_scroll_idx = CLAMP(next, 0, (int)menuItems.size() - 1);
                    } else {
                        auto& item = menuItems[menu_scroll_idx];
    ...
    while (keep_running) {
        {
            ...
            for (uint32_t i = 0; i < num_frames; i++) {
                ...
                buffer[i*2] = v16; buffer[i*2+1] = v16;
            }
        }
            ...
            if (n > 0) {
                for (int i = 0; i < n; i++) {
                    if ((buf[i] & 0xF0) == 0x90 && buf[i+1] == 38 && buf[i+2] > 0) {
                        std::lock_guard<std::mutex> lock(engine_mutex);
                        kick2.noteOn(60, (float)buf[i+2]/127.0f); v_meter = 1.0f;
                    }
                }
            } else if (n < 0 && n != -EAGAIN) { snd_rawmidi_close(midi_h); mc101_connected = false; }
...
int main() {
    ...
    for (int i = 0; i < 12; i++) { shiftParams[i].finalize(); }
    slotParam.finalize();
    ...
    while (emulator.isOpen() && keep_running) {
        ...
        if (!mc101_connected) {
            ...
            for (int i = 0; i < (int)menuItems.size(); i++) {
                int y = 24 + (i * 14);
                std::string line = (i == menu_scroll_idx) ? "> " : "  ";
                line += menuItems[i].label;
                display.text({4, y}, line, {.font = &PoppinsLight_8});
                if (menuItems[i].type == MenuItem::PARAM)
                    display.text({90, y}, formatValue(*menuItems[i].param), {.font = &PoppinsLight_8});
            }
        } 
        else {
            ...
            for (int i = 0; i < 12; i++) {
                ...
                if (col < COLS-1) for (int dY=y; dY<y+CELL_H; dY+=3) display.setPixel({x+CELL_W-1, dY});
                if (row < ROWS-1) for (int dX=x; dX<x+CELL_W; dX+=3) display.setPixel({dX, y+CELL_H-1});
            }
        ...
        // Global Overlays (Status messages & Meter)
        if (msg_timer > 0) {
            display.filledRect({20, 50}, {88, 20});
    ...
    keep_running = false; aThread.join(); mThread.join();
    snd_pcm_close(pcm_h); return 0;
}
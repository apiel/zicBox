...
Param shiftParams[12] = {
    ...
    { "" }, // <---- here we should be able to select the engine
    { "" },
    { "" },
    { "" },
    { "" },
...
Param slotParam { "Select Slot", .value = 1.0f, .min = 1.0f, .max = 99.0f, .step = 1.0f, .precision = 0 };
int active_slot = -1;
...
void loadState(int slot, int messageDuration = 120)
{
    ...
    if (!is) {
        setMessage("PATCH NOT FOUND", messageDuration);
        return;
    }
...
int main()
{
    ...
    menuItems = {
        ...
        // { "Revert (Undo)", MenuItem::ACTION, nullptr, []() { if(active_slot != -1) loadState(active_slot); } },

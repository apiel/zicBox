...
public:
    ...
    Param& duration = addParam(Param{ .key = "duration", .label = "Duration", .unit = "ms", .value = 400.0f, .min = 10.0f, .max = 2000.0f, .step = 10.0f });
    Param& baseFrequency = addParam(Param{ .key = "baseFrequency", .label = "Body Freq", .unit = "Hz", .value = 180.0f, .min = 100.0f, .max = 400.0f });

    Param& bodyDecay = addParam(Param{ .key = "bodyDecay", .label = "Body", .unit = "%", .value = 30.0f });
    Param& ringAmount = addParam(Param{ .key = "ringAmount", .label = "Ring", .unit = "%", .value = 25.0f });

    Param& snappyLevel = addParam(Param{ .key = "snappyLevel", .label = "Snappy", .unit = "%", .value = 50.0f });
    Param& snappyDecay = addParam(Param{ .key = "snappyDecay", .label = "Snap Tail", .unit = "%", .value = 40.0f });
    Param& snapTone = addParam(Param{ .key = "snapTone", .label = "Snap Tone", .unit = "%", .value = 50.0f });

    Param& pitchDrop = addParam(Param{ .key = "pitchDrop", .label = "Punch", .unit = "%", .value = 20.0f });
    Param& impact = addParam(Param{ .key = "impact", .label = "Impact", .unit = "%", .value = 30.0f });

    Param& drive = addParam(Param{ .key = "drive", .label = "Drive", .unit = "%", .value = 15.0f });
    Param& tightness = addParam(Param{ .key = "tightness", .label = "Tightness", .unit = "%", .value = 50.0f });
    Param& tone = addParam(Param{ .key = "tone", .label = "Tone", .unit = "%", .value = 100.0f });

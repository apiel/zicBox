...
protected:
                   ...
                   bool gate)
    {
        ...
        case 2:
            env -= decayRate;
            if (env <= sustainLevel) { env = sustainLevel; stage = 3; }
            break;
        case 3: env = sustainLevel; break;
        case 4:
            env -= releaseRate;
...
public:
    ...
    Param& lfoType    = addParam({ .key = "lfoType", .label = "LFO Type", .string = lfo.typeName, .value = 0.0f, .max = Lfo::COUNT - 1, .target = PG_MOD, .module = MODULE_LFO,
                                   ...
                                   .graph    = [](void* ctx, float val) { return ((Noise23*)ctx)->lfo.graph(val); } });
    Param& lfoRate    = addParam({ .key = "lfoRate", .label = "LFO Rate", .unit = "Hz", .value = 0.3f, .min = 0.01f, .max = 40.0f, .step = 0.01f, .incType = INC_SCALED, .target = PG_MOD, .module = MODULE_LFO,
    ...
    Param& texAttack  = addParam({ .key = "texAttack",  .label = "Tex Attack",  .unit = "ms", .value = 800.0f,  .min = 5.0f,  .max = 8000.0f, .step = 5.0f,  .target = PG_ENV });
    Param& texDecay   = addParam({ .key = "texDecay",   .label = "Tex Decay",   .unit = "ms", .value = 1200.0f, .min = 10.0f, .max = 8000.0f, .step = 5.0f,  .target = PG_ENV });
    Param& texSustain = addParam({ .key = "texSustain", .label = "Tex Sustain", .unit = "%",  .value = 60.0f,                                                 .target = PG_ENV });
    Param& texRelease = addParam({ .key = "texRelease", .label = "Tex Release", .unit = "ms", .value = 3000.0f, .min = 10.0f, .max = 12000.0f,.step = 5.0f,  .target = PG_ENV });
    // How much the texture envelope sweeps BP-A upward (in octaves)
    ...
    Param& fxType    = addParam({ .key = "fxType",    .label = "FX Type",   .string = fxName, .value = 0.0f, .max = (float)MultiFx::FX_COUNT - 1, .step = 1.0f,
                                  .onUpdate   = [](void* ctx, float v)          { auto e = (Noise23*)ctx; e->multiFx.setEffect(v);       strcpy(e->fxName, e->multiFx.getEffectName()); },
                                  .hydrateFn  = [](void* ctx, const char* vStr) { auto e = (Noise23*)ctx; e->multiFx.setEffect(vStr); } });
        ...
        , lfo(sr)
    {
        ...
        if (reverbBuf) {
            int pos = 0;
            for (int c = 0; c < 4; ++c) { rvCombOff[c] = pos; pos += COMB_LEN[c]; }
            for (int a = 0; a < 3; ++a) { rvApOff  [a] = pos; pos += AP_LEN [a]; }

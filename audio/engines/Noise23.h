...
public:
    ...
    Param& fxType = addParam({ .key = "fxType", .label = "FX Type", .string = fxName, .value = 0.0f, .max = (float)MultiFx::FX_COUNT - 1, .step = 1.0f, // Skip Format
        .onUpdate = [](void* ctx, float v) { auto e = (Noise23*)ctx; e->multiFx.setEffect(v); }, // Skip Format
        .setStringFn = [](void* ctx, float value, char* str) { auto e = (Noise23*)ctx; strcpy(str, e->multiFx.getEffectName(value)); }, // Skip Format
        .stringToFloatFn = [](void* ctx, const char* vStr) { auto e = (Noise23*)ctx; return (float)e->multiFx.getEffect(vStr); } }); // Skip Format

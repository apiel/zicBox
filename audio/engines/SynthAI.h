...
public:
    ...
    Param params[47] = {
        { .label = "Osc1 Wave", .string = osc1WaveName, .value = 1.0f, .max = 6.0f, .target = PG_OSC1, .onUpdate = [](void* c, float v) { strncpy(((SynthAI*)c)->osc1WaveName, WAVE_NAMES[(int)v], 15); }, .description = "Waveform for Osc1: 0=Sine, 1=Saw, 2=Square, 3=Triangle, 4=White, 5=Pink, 6=Brown" },
        { .label = "Osc1 Freq", .unit = "hz", .value = 440.0f, .min = 10, .max = 2000.0f, .target = PG_OSC1, .description = "Fundamental frequency of Oscillator 1 in Hz." },
        { .label = "Osc1 Env1 Freq", .value = 0.0f, .min = -48.0f, .max = 48.0f, .target = PG_OSC1, .description = "Modulation depth of Envelope 1 on Osc1 frequency (-48 to +48 semitones)." },
        { .label = "Osc1 Env2 Freq", .value = 0.0f, .min = -48.0f, .max = 48.0f, .target = PG_OSC1, .description = "Modulation depth of Envelope 2 on Osc1 frequency (-48 to +48 semitones)." },
        { .label = "Osc1 LFO1 Freq", .value = 0.0f, .min = -24.0f, .max = 24.0f, .target = PG_OSC1, .description = "Modulation depth of LFO 1 on Osc1 frequency (-24 to +24 semitones)." },
        { .label = "Osc1 LFO2 Freq", .value = 0.0f, .min = -24.0f, .max = 24.0f, .target = PG_OSC1, .description = "Modulation depth of LFO 2 on Osc1 frequency (-24 to +24 semitones)." },
        { .label = "Osc1 Env1 Amp", .value = 100.0f, .min = -100.0f, .max = 100.0f, .target = PG_OSC1, .description = "Amount of Envelope 1 sent to Osc1 amplitude (VCA)." },
        { .label = "Osc1 Env2 Amp", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_OSC1, .description = "Amount of Envelope 2 sent to Osc1 amplitude (VCA)." },
        { .label = "Osc1 LFO1 Amp", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_OSC1, .description = "Amount of LFO 1 sent to Osc1 amplitude (Tremolo)." },
        { .label = "Osc1 LFO2 Amp", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_OSC1, .description = "Amount of LFO 2 sent to Osc1 amplitude (Tremolo)." },

        { .label = "Osc2 Wave", .string = osc2WaveName, .value = 0.0f, .max = 6.0f, .target = PG_OSC2, .onUpdate = [](void* c, float v) { strncpy(((SynthAI*)c)->osc2WaveName, WAVE_NAMES[(int)v], 15); }, .description = "Waveform for Osc2: 0=Sine, 1=Saw, 2=Square, 3=Triangle, 4=White, 5=Pink, 6=Brown" },
        { .label = "Osc2 Ratio", .value = 1.0f, .min = 0.0f, .max = 16.0f, .step = 0.01f, .target = PG_OSC2, .description = "Frequency ratio of Osc2 relative to Osc1." },
        { .label = "Osc2 Env2 Ratio", .value = 0.0f, .min = -8.0f, .max = 8.0f, .target = PG_OSC2, .description = "Modulation depth of Envelope 2 on Osc2 ratio." },
        { .label = "Osc2 LFO2 Ratio", .value = 0.0f, .min = -8.0f, .max = 8.0f, .target = PG_OSC2, .description = "Modulation depth of LFO 2 on Osc2 ratio." },
        { .label = "Osc2 Env1 Amp", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_OSC2, .description = "Amount of Envelope 1 sent to Osc2 amplitude." },
        { .label = "Osc2 Env2 Amp", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_OSC2, .description = "Amount of Envelope 2 sent to Osc2 amplitude." },
        { .label = "Osc2 LFO2 Amp", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_OSC2, .description = "Amount of LFO 2 sent to Osc2 amplitude." },
        { .label = "Osc2 > Osc1 FM", .value = 0.0f, .max = 100.0f, .description = "Intensity of Frequency Modulation from Osc2 into Osc1 phase." },

        { .label = "Cutoff", .value = 80.0f, .max = 100.0f, .target = PG_FILTER, .description = "Filter cutoff frequency (normalized 0-100)." },
        { .label = "Resonance", .value = 10.0f, .max = 100.0f, .target = PG_FILTER, .description = "Filter resonance or Q factor (normalized 0-100)." },
        { .label = "Filter Type", .string = filterType, .value = 1.0f, .min = 1.0f, .max = 11.0f, .step = 1.0f, .target = PG_FILTER, .onUpdate = [](void* ctx, float val) {
             auto* s = (SynthAI*)ctx;
             switch ((int)val) {
             case 2:
        ...
        { .label = "Env1 Cutoff", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_FILTER, .description = "Modulation depth of Envelope 1 on filter cutoff." },
        { .label = "Env2 Cutoff", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_FILTER, .description = "Modulation depth of Envelope 2 on filter cutoff." },
        { .label = "LFO1 Cutoff", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_FILTER, .description = "Modulation depth of LFO 1 on filter cutoff." },
        { .label = "LFO2 Cutoff", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_FILTER, .description = "Modulation depth of LFO 2 on filter cutoff." },
        { .label = "LFO2 Resonance", .value = 0.0f, .min = -100.0f, .max = 100.0f, .target = PG_FILTER, .description = "Modulation depth of LFO 2 on filter resonance." },

        { .label = "LFO1 Type", .string = lfo1TypeName, .value = 0.0f, .max = 12.0f, .target = PG_LFO, .onUpdate = [](void* c, float v) { strncpy(((SynthAI*)c)->lfo1TypeName, LFO_NAMES[(int)v], 15); }, .description = "LFO 1 Shape: 0=Sin, 1=Saw, 2=Tri, 3=Sqr, 4=Sin Trg, 5=Saw Trg, 6=Tri Trg, 7=Sqr Trg, 8=Sin One, 9=Saw One, 10=Tri One, 11=Sqr One, 12=S&H" },
        { .label = "LFO1 Rate", .unit = "Hz", .value = 2.0f, .max = 400.0f, .target = PG_LFO, .description = "Oscillation speed of LFO 1 in Hz." },
        { .label = "LFO2 Type", .string = lfo2TypeName, .value = 0.0f, .max = 12.0f, .target = PG_LFO, .onUpdate = [](void* c, float v) { strncpy(((SynthAI*)c)->lfo2TypeName, LFO_NAMES[(int)v], 15); }, .description = "LFO 2 Shape: 0=Sin, 1=Saw, 2=Tri, 3=Sqr, 4=Sin Trg, 5=Saw Trg, 6=Tri Trg, 7=Sqr Trg, 8=Sin One, 9=Saw One, 10=Tri One, 11=Sqr One, 12=S&H" },
        { .label = "LFO2 Rate", .unit = "Hz", .value = 0.5f, .max = 400.0f, .target = PG_LFO, .description = "Oscillation speed of LFO 2 in Hz." },

        { .label = "Env1 Attack", .unit = "ms", .value = 10.0f, .max = 1000.0f, .step = 10.0f, .target = PG_ENV, .description = "Time for Env 1 to reach peak level (ms)." },
        { .label = "Env1 Decay", .unit = "ms", .value = 100.0f, .max = 1000.0f, .step = 10.0f, .target = PG_ENV, .description = "Time for Env 1 to fall to sustain level (ms)." },
        { .label = "Env1 Sustain", .value = 50.0f, .target = PG_ENV, .description = "Steady state level for Env 1 while gate is open (0-100)." },
        { .label = "Env1 Release", .unit = "ms", .value = 200.0f, .max = 5000.0f, .step = 10.0f, .target = PG_ENV, .description = "Time for Env 1 to fall to zero after gate close (ms)." },

        { .label = "Env2 Attack", .unit = "ms", .value = 10.0f, .max = 1000.0f, .step = 10.0f, .target = PG_ENV, .description = "Time for Env 2 to reach peak level (ms)." },
        { .label = "Env2 Decay", .unit = "ms", .value = 400.0f, .max = 1000.0f, .step = 10.0f, .target = PG_ENV, .description = "Time for Env 2 to fall to sustain level (ms)." },
        { .label = "Env2 Sustain", .value = 0.0f, .target = PG_ENV, .description = "Steady state level for Env 2 while gate is open (0-100)." },
        { .label = "Env2 Release", .unit = "ms", .value = 500.0f, .max = 5000.0f, .step = 10.0f, .target = PG_ENV, .description = "Time for Env 2 to fall to zero after gate close (ms)." },

        { .label = "Drive", .value = 0.0f, .max = 100.0f, .target = PG_FX, .description = "Saturation/distortion gain added to the signal." },
        { .label = "Waveshaper", .value = 0.0f, .max = 100.0f, .target = PG_FX, .description = "Waveshaping amount." },
        { .label = "Decimator", .value = 0.0f, .max = 100.0f, .target = PG_FX, .description = "Sample rate reduction amount for aliasing/bit-crush effects." },
        { .label = "Glide", .value = 0.0f, .max = 500.0f, .target = PG_FX, .description = "Time for pitch to slide between consecutive notes (ms)." },

        { .label = "Reverb Mix", .unit = "%", .value = 0.0f, .target = PG_FX, .description = "Reverb wet signal amount (0-100%)." },
        { .label = "Rvb Damp", .unit = "%", .value = 50.0f, .target = PG_FX, .description = "High frequency absorption in the reverb space." },
        { .label = "Dly Mix", .unit = "%", .value = 0.0f, .target = PG_FX, .description = "Delay wet signal amount (0-100%)." },
        { .label = "Dly Time", .unit = "ms", .value = 125.0f, .min = 10.0f, .max = 1000.0f, .step = 5.0f, .target = PG_FX, .description = "Time interval between delay repeats (ms)." },
        { .label = "Dly Fdbk", .unit = "%", .value = 0.0f, .target = PG_FX, .description = "Amount of delayed signal fed back into the delay line." },
    };

...
public:
    ...
    // Page 2: Pitch Envelope, Transient & Filter
    Param& fmDepth = addParam({ .key = "fmDepth", .label = "FM Depth", .unit = "%", .value = 35.0f, .min = 0.0f, .max = 100.0f, .step = 1.0f });
    Param& pitchDecay = addParam({ .key = "pitchDecay", .label = "Pitch Decay", .unit = "ms", .value = 25.0f, .min = 5.0f, .max = 150.0f, .step = 1.0f });
    Param& transientMorph = addParam({ .key = "transientMorph", .label = "Click Type", .unit = "%", .value = 0.0f, .min = 0.0f, .max = 100.0f, .step = 1.0f });
    Param& noiseColor = addParam({ .key = "noiseColor", .label = "Click Tone", .unit = "%", .value = 60.0f, .min = 0.0f, .max = 100.0f, .step = 1.0f });

    // Page 3: Drive & Sub-Rumble Tail
    ...
    Param& subPitch = addParam({ .key = "subPitch", .label = "Sub Pitch", .unit = "%", .value = 70.0f, .min = 20.0f, .max = 100.0f, .step = 1.0f });
    ...
    float sampleImpl()
    {
        ...
        if (envAmp > 0.0001f) {
            float pDecaySec = std::clamp(pitchDecay.value * 0.001f, 0.005f, 0.150f);
            modulationEnvelope *= Math::exp(-1.0f / (sampleRate * pDecaySec));
        ...
        // 2. Deep Sub-Bass Pitch-Down Buffer Replay
        ...
        if (rAmt > 0.001f) {
            ...
            if (kickElapsedSamples >= targetGapSamples) {
                ...
                if (idxA < kickWritePos) {
                    ...
                    float replaySpeed = std::clamp(subPitch.value * 0.01f, 0.20f, 1.00f);
                    kickReadPos += replaySpeed;
                }
        ...
        // 4. Attack Transient Click Generation & Filter Shaping
        if (clickEnvelope > 0.0001f) {
            clickEnvelope *= Math::exp(-1.0f / (sampleRate * 0.010f)); // 10ms transient click

            float tMorph = transientMorph.value * 0.01f;
            float clickRaw = 0.0f;

            if (tMorph < 0.33f) {
                // Pure Noise Click
                clickRaw = nextNoise();
            } else if (tMorph < 0.66f) {
                // Square Impulse Click
                clickRaw = (nextNoise() > 0.0f) ? 0.9f : -0.9f;
            } else {
                // Sine Pitch Pop
                float popPh = (1.0f - clickEnvelope) * 4.0f;
                clickRaw = Math::fastSin2(PI_X2 * popPh);
            }

            // Transient Filter Shaping (noiseColor)
            float colCutoff = lerp(300.0f, 16000.0f, noiseColor.value * 0.01f);
            float alpha = std::clamp(PI_X2 * colCutoff / sampleRate, 0.01f, 0.99f);
            clickLP += alpha * (clickRaw - clickLP);

            float clickSig = clickLP * clickEnvelope * (pct(clickAmt) * 0.75f);
            out += clickSig;
        }

...
public:
    ...
    Param& fxType = addParam({ .key = "fxType", .label = "FX Type", .string = fxName, .value = 0.0f, .max = (float)MultiFx::FX_COUNT - 1, .step = 1.0f, // Skip Format
        .onUpdate = [](void* ctx, float v) { auto e = (DrumGeneric*)ctx; e->multiFx.setEffect(v); }, // Skip Format
        .setStringFn = [](void* ctx, float value, char* str) { auto e = (DrumGeneric*)ctx; strcpy(str, e->multiFx.getEffectName(value)); }, // Skip Format
        .stringToFloatFn = [](void* ctx, const char* valStr) { auto e = (DrumGeneric*)ctx; return (float)e->multiFx.getEffect(valStr); } }); // Skip Format
    ...
    {
        // 1. Kick body waveform & decay envelope
        float durBody = bodyDuration.value / 2000.0f;
        float envBody = std::exp(-x * (3.0f + (1.0f - durBody) * 8.0f));
        float pSweep = std::exp(-x * (5.0f + pct(sweepDep) * 15.0f));
        float ph = x * (2.0f + pSweep * 8.0f);
        float bodySig = getBodyWave(ph - std::floor(ph), pct(bodyMorph)) * envBody;

        // 2. Clap / Hi-Hat noise layer
        float durNoise = hiClapDuration.value / 2000.0f;
        float envNoise = std::exp(-x * (4.0f + (1.0f - durNoise) * 10.0f));
        float noiseSig = (std::sin(x * 120.0f) * std::cos(x * 250.0f)) * envNoise;

        // 3. Simple layer mix (-100 = 100% Body, +100 = 100% Hi/Clap)
        float blend = (mix.value + 100.0f) * 0.005f;
        float sig = bodySig * (1.0f - blend) + noiseSig * blend;

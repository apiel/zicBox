...
// 4. Texture (Replaces VCO2 - acts as kick harmonic body on-beat, or synth/stabs off-beat)
...
public:
    ...
    Param& synthCutoff = addParam({ .key = "synthCutoff", .label = "Cutoff", .unit = "", .value = 0.3f, .min = 0.02f, .max = 0.98f });
    Param& synthResonance = addParam({ .key = "synthResonance", .label = "Reso", .unit = "", .value = 0.5f, .min = 0.0f, .max = 0.99f });
    ...
    Param& synthDrive = addParam({ .key = "synthDrive", .label = "Tex Drv", .unit = "", .value = 0.0f, .min = 0.0f, .max = 1.0f });
    ...
    Param& mix = addParam({ .key = "mix", .label = "Tex Mix", .unit = "", .value = 0.4f, .min = 0.0f, .max = 1.0f });
    ...
    // Trigger Kick Body & Click (and trigger Texture if on-beat)
    ...
    // Trigger Texture (replaces VCO2 - called on Texture sequencer steps)
    ...
    float sampleImpl() {
        ...
        // --- 2. Texture Generation (Replaces VCO 2) ---
        float textureOut = 0.0f;
        if (synthAmpEnv > 0.001f) {
            float synthReleaseCoeff = std::exp(-1.0f / (sampleRate * (synthRelease.value * 0.001f)));
            synthAmpEnv *= synthReleaseCoeff;

            synthPhase += synthTargetFreq * sampleRateDiv;
            if (synthPhase > 1.0f) synthPhase -= 1.0f;

            float rawSynth = getVCO(synthPhase, synthWaveform.value);
            
            // Filter
            float cutoffMod = std::clamp(synthCutoff.value + (synthAmpEnv * synthEnvAmt.value), 0.01f, 0.98f);
            synthFilter.setCutoff(cutoffMod);
            synthFilter.setResonance(synthResonance.value);
            textureOut = synthFilter.process12(rawSynth).lp * synthAmpEnv;

            if (synthDrive.value > 0.001f) {
                textureOut = std::tanh(textureOut * (1.0f + synthDrive.value * 8.0f));
            }
            ...
            if (synthDelayMix.value > 0.001f) {
                ...
                delayBuf[delayWrite] = textureOut + delayVal * dlyFbSmooth;
                delayWrite = (delayWrite + 1) % DELAY_BUF_SIZE;
                textureOut = lerp(textureOut, textureOut + delayVal, synthDelayMix.value);
            }
        }
        ...
        float summed = kickOut * kickGain + textureOut * texGain;
    ...
    float drawImpl(float x) {
        return getVCO(x, synthWaveform.value);
    }

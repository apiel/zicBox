...
function saveHeader(filename: string, arrayName: string, data: number[]) {
    const content =
...
saveHeader('sineTable.h', 'SINE_TABLE', sine);
...
saveHeader('tanhTable.h', 'TANH_TABLE', tanh);
...
saveHeader('expTable.h', 'EXP_TABLE', exp);
...
saveHeader('inverseTable.h', 'INV_TABLE', inv);

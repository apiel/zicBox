...
void dispatchHardwareEncoderEvent(UiPixelDrift& ui, int encoderId, int8_t direction, bool& needFullRedraw)
{
...
void dispatchHardwareKeyEvent(UiPixelDrift& ui, int key, bool pressed, bool& needFullRedraw)
{
    char k = (char)key;
    if (k == 'q' || k == 'w' || k == 'e' || k == 'r' || k == 'f') {
        if (pressed) {
            ui.handleViewKey(k, needFullRedraw);
        }
    } else {
        ui.handlePerformancePad(k, pressed, needFullRedraw);
    }
}
}
...
void runHardware(Draw& d, UiPixelDrift& ui, bool& needFullRedraw)
{
    ...
    // Default key definitions matching PixelController.h pixel_12btn layout
    ...
    // Default encoder definitions matching PixelController.h pixel_12btn layout
    ...
    // Load optional GPIO override configuration from config.json
    ...
    if (configFile.is_open()) {
        try {
            ...
            logWarn("Failed to parse config.json, using pixel_12btn defaults.");
        }
    }
    ...
    if (gpioKey.init() == 0) {
        gpioKey.startThread();
        logInfo("Hardware keys initialized (GPIO pixel_12btn)");
    } else {
    ...
    if (gpioEncoder.init() == 0) {
        ...
        logInfo("Hardware encoders initialized (GPIO pixel_12btn)");
    } else {

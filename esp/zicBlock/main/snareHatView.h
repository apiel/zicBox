...
public:
    ...
    void render() override
    {
        ...
        // renderBar(valuePos[5], audio.snareHat.noiseMix);
        // renderStringValue(valuePos[5], "Noise", std::to_string((int)(audio.snareHat.noiseMix * 100)) + "%");

        // renderCenteredBar(valuePos[6], audio.snareHat.punch);
        // renderStringValue(valuePos[6], "Punch", std::to_string((int)(fabs(audio.snareHat.punch) * 100)) + "%");
    ...
    void onEncoder(int id, int8_t direction, uint64_t tick) override
    {
        if (id == 1) {
            ...
            // audio.snareHat.setNoiseMix(audio.snareHat.noiseMix + direction * 0.01f);
        } else if (id == 7) {
            // audio.snareHat.setPunch(audio.snareHat.punch + direction * 0.01f);
        } else if (id == 8) {

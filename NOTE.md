...
- TODO utilise all CPU core
    ...
    ```
    - use pthread to set the priority of the audio thread to real-time:
    ...
    void set_realtime_priority() {
        ...
        params.sched_priority = 20; // Adjust based on system limits
        pthread_setschedparam(pthread_self(), SCHED_FIFO, &params);
    }
    ```
    - Run the application with real-time scheduling policies: `sudo chrt -f 20 ./your_application`
    ...
    // Array to hold generated samples
    float track_samples[NUM_TRACKS];

    // Function to generate a sample for a track
    void generate_sample(int track_id) {
        // Simulate generating a sample
        track_samples[track_id] = static_cast<float>(track_id + 1); // Example value
        std::cout << "Track " << track_id << " generated sample: " << track_samples[track_id] << "\n";
    }

    // Function to mix samples
    float mix_samples() {
        float mixed_sample = 0.0f;
        for (int i = 0; i < NUM_TRACKS; ++i) {
            mixed_sample += track_samples[i]; // Simple mixing by summation
        }
        return mixed_sample;
    }

    int main() {
        // Create a barrier for 4 threads + 1 (main thread to mix samples)
        std::barrier sync_point(NUM_TRACKS, [] {
            // Mixing task: This runs when all threads reach the barrier
            float result = mix_samples();
            std::cout << "Mixed sample: " << result << "\n";
        });

        // Launch threads for each track
        std::vector<std::thread> threads;
        for (int i = 0; i < NUM_TRACKS; ++i) {
            threads.emplace_back([i, &sync_point] {
                generate_sample(i);
                sync_point.arrive_and_wait(); // Synchronize here
            });
        }
    ...
    constexpr int NUM_TRACKS = 4;
    constexpr int BUFFER_SIZE = 128; // Number of samples requested by PulseAudio
    float track_samples[NUM_TRACKS][BUFFER_SIZE];
    float mixed_samples[BUFFER_SIZE];
    ...
    void generate_sample(int track_id, int sample_id) {
        // Simulate generating a sample
        track_samples[track_id][sample_id] = static_cast<float>(track_id + 1 + sample_id * 0.1f);
    }

    void mix_samples(int sample_id) {
        mixed_samples[sample_id] = 0.0f;
        for (int track_id = 0; track_id < NUM_TRACKS; ++track_id) {
            mixed_samples[sample_id] += track_samples[track_id][sample_id];
        }
    }
    ...
    void track_worker(int track_id) {
        ...
        while (!stop_flag) {
            // Wait for a signal to generate the next sample
            {
                ...
                cv.wait(lock, [track_id, sample_id] { return ready_count == sample_id; });
            }

            // Generate the sample for the current track
            generate_sample(track_id, sample_id);
    ...
    int main() {
        ...
        for (int sample_id = 0; sample_id < BUFFER_SIZE; ++sample_id) {
            ...
            // Mix the samples
            mix_samples(sample_id);
            std::cout << "Mixed sample " << sample_id << ": " << mixed_samples[sample_id] << "\n";
        }

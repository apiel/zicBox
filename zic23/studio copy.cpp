...
static sf::IntRect eqHeaderBtnRect_unused; // kept for completeness
...
// Helpers
// ================================================================
void triggerPreview(Track& trk, int note, float vel, int ms = 200)
{
    {
        std::lock_guard<std::mutex> lk(studio.audioMutex);
        trk.engine->noteOn(note, vel);
    }
    std::thread([&trk, note, ms]() {
...
// ================================================================
// drawStaticUI
// Sets all rect globals that event handlers and pixel updaters need.
// ================================================================
void drawStaticUI(Draw& d, sf::Vector2u size)
{
    ...
    // ---- EQ editor zone ----------------------------------------
    //
    // Layout inside the EQ zone:
    //   [MARGIN .. MARGIN+EQ_TRACK_W]          = track selector column
    //   [specRects[0].left .. specRects[0].left+SPEC_W] = EQ curve (aligned with spectrum strips)
    //
    // The EQ curve X-origin and width are taken directly from the spectrum strip rects
    // so frequencies are pixel-perfectly aligned.
    //
    ...
    // specRects[0].left is where all spectrum strips start (they share the same X)
    int eqCx = specRects[0].left;
    int eqCw = SPEC_W; // same width as spectrum strip
    int eqCy = eqZoneTop + 10;
    int eqCh = EQ_ZONE_H - 20;

    // Store for mouse handler and pixel updater
    ...
    // Frequency grid — same log positions as spectrum columns
...
// ================================================================
// updateWaveforms  — into pixel buffer (unchanged pattern)
// ================================================================
...
// ================================================================
// updateSequencerPixels  — into pixel buffer
// ================================================================
...
// ================================================================
// updateSpectrumPixels  — runs FFT + draws strips into pixel buffer
// Called every frame. Because it reads post-EQ ring buffer samples,
// it always reflects the current EQ state automatically.
// ================================================================
                    ...
                    if (studio.bpmRect.contains(mx, my)) {
                        ...
                        for (auto& trk : studio.tracks) {
                            if (trk->volRect.contains(mx, my)) {
                                ...
                                // winW, MARGIN, ROW_H are all properly scoped here

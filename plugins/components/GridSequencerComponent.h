...
public:
    /*md **Keyboard actions**: */
    void addKeyMap(uint16_t controllerId, uint8_t key, int param, std::string action, uint8_t color)
    {
        ...
        if (action == "track") {
            currentKeypadLayout->mapping.push_back({ controllerId, key, param, [&](int8_t state, KeypadLayout::KeyMap& keymap) { updateTrackSelection(state, keymap.param); }, color, [&](KeypadLayout::KeyMap& keymap) { return tracks[keymap.param].status->get() == 1 ? 40 : 0; } });
            ...
            currentKeypadLayout->mapping.push_back({ controllerId, key, param, [&](int8_t state, KeypadLayout::KeyMap& keymap) { updateParamSelection(state, keymap.param); }, color, [&](KeypadLayout::KeyMap& keymap) { return keymap.color; } });
            /*md - `row` to select row number: `KEYMAP: 1 row -1` will decrement the current row selection when key 1 is pressed. */
        } else if (action == "row") {
            currentKeypadLayout->mapping.push_back({ controllerId, key, param, [&](int8_t state, KeypadLayout::KeyMap& keymap) { updateRowSelection(state, keymap.param); }, color, [&](KeypadLayout::KeyMap& keymap) { return 20; } });
            /*md - `col` to select column number: `KEYMAP: 1 col -1` will decrement the current column selection when key 1 is pressed. */
        } else if (action == "col") {
            currentKeypadLayout->mapping.push_back({ controllerId, key, param, [&](int8_t state, KeypadLayout::KeyMap& keymap) { updateColSelection(state, keymap.param); }, color, [&](KeypadLayout::KeyMap& keymap) { return 20; } });
            /*md - `master` to select master track: `KEYMAP: 1 master` will select master when key 1 is pressed. */
        } else if (action == "master") {
            currentKeypadLayout->mapping.push_back({ controllerId, key, param, [&](int8_t state, KeypadLayout::KeyMap& keymap) { updateMasterSelection(state); }, color, [&](KeypadLayout::KeyMap& keymap) { return 40; } });
            /*md - `variation` to select variation: `KEYMAP: 1 variation` will select variation when key 1 is pressed. */
        } else if (action == "variation") {
            currentKeypadLayout->mapping.push_back({ controllerId, key, param, [&](int8_t state, KeypadLayout::KeyMap& keymap) { tracks[grid.row].variation->set(keymap.param); }, color, [&](KeypadLayout::KeyMap& keymap) { return 60; } });
            /*md - `step` to update a step: `KEYMAP: 1 step 4` will update step 4 when key 1 is pressed. */
        } else if (action == "step") {
            currentKeypadLayout->mapping.push_back({ controllerId, key, param, [&](int8_t state, KeypadLayout::KeyMap& keymap) { updateStepSelection(state, keymap.param); }, color, [&](KeypadLayout::KeyMap& keymap) { 
                if (grid.row >= trackCount) {
                    return 254;
                }
            ...
            currentKeypadLayout->mapping.push_back({ controllerId, key, param, [&](int8_t state, KeypadLayout::KeyMap& keymap) { updateLayout(state, keymap.param); }, color, [&](KeypadLayout::KeyMap& keymap) { return keymap.color == 255 ? 90 : keymap.color; } });
            /*md - `none` to disable keypad button: `KEYMAP: 1 none` will disable the button 1. */
        } else if (action == "none") {
            currentKeypadLayout->mapping.push_back({ controllerId, key, param, [&](int8_t state, KeypadLayout::KeyMap& keymap) {}, color, [&](KeypadLayout::KeyMap& keymap) { return 254; } });
        }
    }
    ...
    bool config(char* key, char* value)
    {
        ...
        if (strcmp(key, "KEYPAD_LAYOUT") == 0) {
            currentKeypadLayout = new KeypadLayout((KeypadInterface*)getController("Keypad"), [&](uint16_t controllerId, int8_t state, int param, std::string action, uint8_t color) { addKeyMap(controllerId, state, param, action, color); });

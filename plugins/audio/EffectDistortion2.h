...
public:
    ...
    float sineLookupInterpolated(float x)
    {
        ...
        float scaledIndex = x * (props.lookupTable->size - 1);
        int index = static_cast<int>(scaledIndex);
        float fraction = scaledIndex - index;

        // Linear interpolation
        float value1 = props.lookupTable->sine[index];
        float value2 = props.lookupTable->sine[(index + 1) % props.lookupTable->size]; // Wrap around for periodicity

...
public:
    // Filter modes
    enum FilterMode { MODE_LP = 0, MODE_BP, MODE_HP, MODE_COUNT };
    ...
    // Filter 2
    Val& cutoff2 = val(2000.0f, "CUTOFF2", { "Filter2.|Cutoff", .min = 20.0f, .max = 20000.0f, .step = 20.0f, .unit = "Hz" }, [&](auto p) {
        p.val.setFloat(p.value);
        needUpdateCoeffs2 = true;
    });

    Val& res2 = val(0.5f, "RES2", { "Filter2.|Resonance", .min = 0.01f, .max = 1.5f, .step = 0.01f, .floatingPoint = 2,.unit = "Q" }, [&](auto p) {
        p.val.setFloat(p.value);
        needUpdateCoeffs2 = true;
    });

    Val& mode2 = val((float)MODE_LP, "MODE2", { "Filter2.|Mode", .max = MODE_COUNT - 1, .skipJumpIncrements = true }, [&](auto p) {
        p.val.setFloat(p.value);
        needUpdateCoeffs2 = true;
    });
    ...
    Val& feedback = val(0.0f, "FEEDBACK", { "Routing.|Feedback", .min = -95.0f, .max = 95.0f, .unit = "%" }, [&](auto p) {
        p.val.setFloat(p.value);
        feedbackAmount = p.val.get() / 100.0f;
    });

    // Routing: 0 = Serial, 1 = Parallel
    Val& routing = val(0.0f, "ROUTING", { "Routing", VALUE_STRING, .max = 1.0f, .skipJumpIncrements = true }, [&](auto p) {
        p.val.setFloat(p.value);
        routingSerial = (p.val.get() <= 0.5f);
        p.val.setString(routingSerial ? "Serial" : "Parallel");
    });
    ...
    void updateCoeffsIfNeeded()
    {
        if (needUpdateCoeffs1) {
            ...
            if (mode == MODE_LP) filter1.setLP(fc, Q, sr);
            else if (mode == MODE_BP) filter1.setBP(fc, Q, sr);
        ...
        if (needUpdateCoeffs2) {
            ...
            int mode = (int)mode2.get();
            if (mode == MODE_LP) filter2.setLP(fc, Q, sr);
            else if (mode == MODE_BP) filter2.setBP(fc, Q, sr);

/** Description:
This code defines the structure for an advanced audio processing module called "SynthLoop," designed to load, loop, and heavily manipulate sound samples using various effects. It functions much like a specialized digital instrument or effect plugin.

The core functionality involves reading an audio file into a large, dedicated memory space (a buffer) and providing a file browser to easily switch between samples. Users can define custom start and end points for the loop playback within the loaded sample.

The sound is processed through a sophisticated chain:

1.  **Main EQ:** The primary looped signal first passes through an Equalizer (EQ) to shape its frequency response.
2.  **Granular Synthesis:** A secondary, powerful engine called "Grains" takes the sample and chops it into tiny fragments. Users gain deep creative control over this process, including adjusting the duration of the fragments, their density (how many play simultaneously), pitch randomization, and playback direction (forward, backward, or random).
3.  **Grain Effects:** The processed granular signal has its own dedicated EQ and is then fed through an additional Multi-Effects unit.

The final sound is generated by mixing the Main EQ output with the enhanced Granular Effects output, allowing seamless blending between the original loop and the textural effects.

All major settings—such as loop boundaries, EQ center frequencies, grain density, and effect mix—are exposed as user-controllable parameters. The module also adapts its playback speed based on musical pitch information (like MIDI notes). It can also optionally randomize playback by dividing the loop into small "chunks."

sha: 1827a408d6bccd472c8204212201f2256e9a4c09de3dcd813c26368655b88b36 
*/
#pragma once

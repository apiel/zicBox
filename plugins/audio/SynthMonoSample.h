...
public:
    ...
    Val& sustainPosition = val(0.0f, "LOOP_POSITION", { "Loop position", .step = 0.1f, .unit = "%" }, [&](auto p) {
        if (p.value < start.get()) {
            p.value = start.get();
        }
    ...
    Val& sustainLength = val(0.0f, "LOOP_LENGTH", { "Loop length", .step = 0.1f, .unit = "%" }, [&](auto p) {

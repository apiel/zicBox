...
class FmAmEngine : public SampleEngine {
protected:
    MultiFx multiFx;

    // Parameters
    Val& mode = val(0, "MODE", { "Mode", VALUE_STRING, .max = 1 }, [&](auto p) { 
        p.val.setFloat(p.value); 
        p.val.setString(p.val.get() ? "FM" : "AM");
    });
...
public:
    FmAmEngine(AudioPlugin::Props& props, AudioPlugin::Config& config, SampleBuffer& sampleBuffer)
        : SampleEngine(props, config, sampleBuffer, "FM/AM")

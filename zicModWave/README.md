...
`zicModWave` is a compact, performance-focused Eurorack/desktop synthesizer module powered by the **`DriftSynth1`** synthesis engine (`audio/engines/DriftSynth1.h`) for the **Waveshare ESP32-S3 Touch LCD 1.47-inch Board** (`Waveshare ESP32-S3-Touch-LCD-1.47`) and **Linux Desktop Simulator**. It features 10 dedicated analog potentiometers, 1 push rotary encoder, real-time MIDI clock/notes synchronization, PDM audio output, and an interactive LCD interface.
...
* **100% Hardware Pin Optimization:**
  * **10 Analog Potentiometers** arranged in a 3x4 grid mapped directly to key synthesis parameters for instant hands-on sound design without menu diving.
  ...
  * **1 PDM Audio Output Pin**: Single-pin 16-bit 44.1 kHz PDM I2S audio output on **GPIO 0**.
...
|-----------|-------------|----------------|
| **Microcontroller Board** | Waveshare ESP32-S3-Touch-LCD-1.47 Board | Core Processor & Display |
| **Display** | ST7789 172x320 Color LCD Display | MOSI: `45`, SCLK: `40`, CS: `42`, DC: `41`, RST: `39`, BL: `46` |
...
Parameters mapped to the 10 physical potentiometers **do not appear in the encoder menu**:
...
| **Pot 9** | `Mod Depth` | Modulation depth | -100% to +100% |
| **Pot 10** | `Mod Speed` | LFO modulation speed | 0% to 100% |
| **Encoder Menu 1** | `Mod Type` | Modulation matrix routing | 16 Matrix Routings |
| **Encoder Menu 2** | `Dly Send` | Delay FX send level | 0% to 100% |
| **Encoder Menu 3** | `Volume` | Master output gain | 0% to 100% |
| **Encoder Menu 4** | `BPM` | Internal BPM / MIDI sync status | 40 to 240 BPM |
| **Encoder Menu 5** | `PLAY / STOP` | Audio playback toggle | `PLAYING` / `STOPPED` |

---

## Desktop Simulator 3x4 Layout

```
+-----------------------------------------------------------------------------------+
|                          1.47-INCH LCD DISPLAY (320x172)                          |
+-----------------------------------------------------------------------------------+
| [Pot 1] Pitch     | [Pot 2] Wave      | [Pot 3] Cutoff     | [EMPTY]              |
+-------------------+-------------------+--------------------+----------------------+
| [Pot 4] Reso      | [Pot 5] Release   | [Pot 6] Env Amt    | [Encoder & Push SW]  |
+-------------------+-------------------+--------------------+----------------------+
| [Pot 7] FiltMorph | [Pot 8] Crsh / FM | [Pot 9] Mod Depth  | [Pot 10] Mod Speed   |
+-----------------------------------------------------------------------------------+
```
